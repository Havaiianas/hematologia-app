    ...
    <div style={{ paddingBottom: 120, minHeight: '100%', position: 'relative' }}>
      ...
      <div style={{ padding: '0 20px', marginTop: -50, position: 'relative' }}>
        <div style={{
          ...
          background: `linear-gradient(135deg, ${COLORS.red}, #6A1A12)`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{ marginTop: 12 }}>
          ...
          <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: COLORS.muted, letterSpacing: 0.8, marginTop: 3 }}>
            {specialty.toUpperCase()} · {crbio}
          </div>
        </div>
        <div style={{
          marginTop: 16, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr',
          background: COLORS.bg2, border: `0.5px solid ${COLORS.line2}`, borderRadius: 14, overflow: 'hidden',
        }}>
          {[{ n: '47', l: 'Análises' }, { n: '12', l: 'Publicados' }, { n: '98%', l: 'Precisão' }].map((s, i) => (
            <div key={i} style={{
              padding: '14px 10px', textAlign: 'center',
              borderRight: i < 2 ? `0.5px solid ${COLORS.line}` : 'none',
            }}>
        ...
        <div style={{ marginTop: 18 }}>
          <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: COLORS.dim, letterSpacing: 1.4, textTransform: 'uppercase', marginBottom: 10 }}>· Conquistas</div>
          <div style={{ display: 'flex', gap: 8, overflowX: 'auto', scrollbarWidth: 'none' }}>
            {[
              { icon: '🔬', t: 'Analista Expert', s: '40+ análises' },
              { icon: '🧬', t: '50 blastos', s: 'detectados' },
              { icon: '⭐', t: 'Contribuidor', s: 'da comunidade' },
            ].map((b, i) => (
              <div key={i} style={{
                flexShrink: 0, padding: '12px 14px', borderRadius: 12,
                background: COLORS.bg2, border: `0.5px solid ${COLORS.line2}`, minWidth: 120,
              }}>
                <div style={{ fontSize: 22 }}>{b.icon}</div>
                <div style={{ fontFamily: FONT_SANS, fontSize: 12, color: COLORS.white, fontWeight: 600, marginTop: 6 }}>{b.t}</div>
                <div style={{ fontFamily: FONT_MONO, fontSize: 9, color: COLORS.dim, marginTop: 2 }}>{b.s}</div>
              </div>
            ))}
          </div>
        </div>
